...
public class GameplayDataTouchToMove : MonoBehaviour
{

    //GAMEOBJECTS
    public GameObject raft;
    private MoveRaftWithTouch moveRaftScript;
    public GameObject raftSails;
    private MoveRaftWithTouch moveRaftSailsScript;
    public GameObject player;
    //private PlayerController playerControllerScript;
    private TouchToMove touchToMoveScript;
    private PlayerHealthTouchToMove playerHealthScript;
    public GameObject coinPouch;
    private CoinPouchScriptTouchToMove coinPouchScript;
    public GameObject chest;
    private ChestScript chestScript;
    private CoinSystem coinSystem;
    public GameObject pausePanel;
    private PauseMenuTouchToMove pauseMenuScript;
    ...
    //PELAAJA
    public int destinationMarkerCounter;
    public int destinationMarkerOnRaftCounter;
    ...
    void Start()
    {
        ...
        moveRaftScript = raft.GetComponent<MoveRaftWithTouch>();
        touchToMoveScript = player.GetComponent<TouchToMove>();
        playerHealthScript = player.GetComponent<PlayerHealthTouchToMove>();
        coinPouchScript = coinPouch.GetComponent<CoinPouchScriptTouchToMove>();
        chestScript = chest.GetComponent<ChestScript>();
        coinSystem = this.GetComponent<CoinSystem>();
        pauseMenuScript = pausePanel.GetComponent<PauseMenuTouchToMove>();
        moveRaftSailsScript = raftSails.GetComponent<MoveRaftWithTouch>();
    ...
    void Update()
    {
        ...
        destinationMarkerOnRaftCounter = moveRaftScript.GetDestinationMarkerCounter() + moveRaftSailsScript.GetDestinationMarkerCounter();

        //PELAAJA
        //playerInWaterTimer = playerControllerScript.GetInWaterTimer();
        destinationMarkerCounter = touchToMoveScript.GetDestinationMarkerCounter();
    ...
    IEnumerator Post(string game, string idnumber, string playtime, string rafttimer, string playerinwatertimer, string raftactivecounter, string raftupgraded, string destinationmarkercounter, string destinationmarkeronraftcounter, string currenthealth, string coconuthits, string coconutsthrown, string pineapplescollected, string deathcounter, string totalcoins, string totaldistance, string timespaused, string gamewon)
    {
        ...
        form.AddField("entry.1042608071", destinationmarkercounter);
        form.AddField("entry.1396738585", destinationmarkeronraftcounter);
        ...
        if(www.isNetworkError)
        {
            ...
            gridLayout.transform.GetChild(7).GetComponent<TMP_Text>().text += " " + destinationmarkercounter;
            gridLayout.transform.GetChild(8).GetComponent<TMP_Text>().text += " " + destinationmarkeronraftcounter;
            gridLayout.transform.GetChild(9).GetComponent<TMP_Text>().text += " " + currenthealth;
            gridLayout.transform.GetChild(10).GetComponent<TMP_Text>().text += " " + coconuthits;
            gridLayout.transform.GetChild(11).GetComponent<TMP_Text>().text += " " + coconutsthrown;
            gridLayout.transform.GetChild(12).GetComponent<TMP_Text>().text += " " + pineapplescollected;
            gridLayout.transform.GetChild(13).GetComponent<TMP_Text>().text += " " + deathcounter;
            gridLayout.transform.GetChild(14).GetComponent<TMP_Text>().text += " " + totalcoins;
            gridLayout.transform.GetChild(15).GetComponent<TMP_Text>().text += " " + totaldistance;
            gridLayout.transform.GetChild(16).GetComponent<TMP_Text>().text += " " + timespaused;
            gridLayout.transform.GetChild(17).GetComponent<TMP_Text>().text += " " + gamewon;
        }else
        {
    ...
    public void SendDataToGoogleForms()
    {
        ...
        DestinationMarkerCounter = destinationMarkerCounter.ToString();
        DestinationMarkerOnRaftCounter = destinationMarkerOnRaftCounter.ToString();
        ...
        StartCoroutine(Post(Game, id, PlayTime, RaftTimer, PlayerInWaterTimer, RaftActiveCounter, RaftUpgraded, DestinationMarkerCounter, DestinationMarkerOnRaftCounter, CurrentHealth, CoconutHits, CoconutsThrown, PineapplesCollected, DeathCounter, TotalCoins, TotalDistance, TimesPaused, GameWon));
    }
